...
enum ServerStatusCode {
  ...
  nonAuthoritativeInformation._(203),
  ...
  proxyAuthenticationRequired._(407),
  ...
  gatewayTimeout._(504),

  /// The HTTP version used in the request is not supported by the server.
  httpVersionNotSupported._(505),

  /// The server has an internal configuration error: the chosen variant resource is configured to engage in transparent content negotiation itself, and is therefore not a proper end point in the negotiation process.
  variantAlsoNegotiates._(506),

  /// The method could not be performed on the resource because the server is unable to store the representation needed to successfully complete the request.
  insufficientStorage._(507),
  ...
  networkAuthenticationRequired._(511);

  /// The code of the server response status
  final int code;
  const ServerStatusCode._(this.code);

  static Map<int, ServerStatusCode> _codes = {
    for (var v in ServerStatusCode.values) v.code: v
  };

  factory ServerStatusCode(int code) {
    assert(_codes.containsKey(code));
    return _codes[code]!;
  }

  bool _inRangeOf(int start) => code >= start && code < start + 100;
  bool get informational => _inRangeOf(100);
  bool get successful => _inRangeOf(200);
  bool get redirectional => _inRangeOf(300);
  bool get clientError => _inRangeOf(400);
  bool get serverError => _inRangeOf(500);

  ServerStatusCodeGroup get codeGroup => informational
      ? ServerStatusCodeGroup.informationalResponse
      : successful
          ? ServerStatusCodeGroup.successfullResponse
          : redirectional
              ? ServerStatusCodeGroup.redirectionMessage
              : clientError
                  ? ServerStatusCodeGroup.clientErrorResponse
                  : ServerStatusCodeGroup.serverErrorResponse;
}
